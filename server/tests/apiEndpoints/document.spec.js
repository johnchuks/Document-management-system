...
describe('Documents', () => {
  ...
  describe('/POST Document', () => {
    it('should add a new document if the user is logged in', (done) => {
      const document = {
        title: 'kiba-team',
    ...
    it('should fail to add a new document if the user is not logged in', (done) => {
      const document = {
        title: 'kiba-team',
        content: 'Andela is really awesome !!!',
        value: 'private',
  ...
  describe('/GET Documents', () => {
    ...
    it('should fail to get all documents if the user is not logged in', (done) => {
      chai.request(server)
      .get('/api/documents/')
      .end((err, res) => {
        expect(res.status).to.equal(403);
  ...
  describe('/GET/:id Document', () => {
   ...
   describe('/PUT/:id, Document', () => {
    ...
    it('Should update a document by id if the user has admin access',
     (done) => {
      const id = 2;
      chai.request(server)
        .put(`/api/documents/${id}`)
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.be.a('object');
          expect(res.body).to.have.property('id').to.equal(2);
          expect(res.body).to.have.property('title').to.equal('narruto');
          done();
        });
    });
